...
public class CheckRunner implements Runnable {
    ...
    public final void run() {
        ...
        if (timeSinceLastCheck < check.getPollingInterval().longValue()) {
            return;
        }
        ...
        try {
            ...
            BigDecimal streak = BigDecimal.ONE;
            ...
            for (Entry<String, Optional<BigDecimal>> entry : targetValues.entrySet()) {
                ...
                if (stateIsTheSame(lastState, currentState)) {
                    streak = streak.add(BigDecimal.ONE);
                    alert.setStreak(streak);
                } else {
                    streak = BigDecimal.ONE;
                    alert.setStreak(streak);
                }
            ...
            for (Subscription subscription : updatedCheck.getSubscriptions()) {
                if (!subscription.shouldNotify(now, worstState, streak)) {
                    continue;
                }
